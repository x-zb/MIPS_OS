...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	for (i = 0; i < size; i += BY2PG) {
		...
		*pgtable_entry = (pa + i) | (perm | PTE_V);
		pgdir[PDX(va_temp)] = pgdir[PDX(va_temp)] | perm; // TODO.
	}
}
...
page_alloc(struct Page **pp)
{
	...
	/* Step 1: Get a page from free memory. */
...
page_free(struct Page *pp)
{
	...
	/* Step 3: If the value of `pp_ref` less than 0, some error must occurred before, 
	 * so PANIC !!! */
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if (ppte) {
		*ppte = (Pte *)(&pgtable[PTX(va)]); // successfully.
	}
