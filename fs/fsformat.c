...
struct Block {
    uint8_t data[BY2BLK]; //数据块内的数据，字节数组
    uint32_t type;  //数据块类型
} disk[NBLOCK];    //disk是管理数据块的数组，每个元素管理一个数据块
...
void init_disk() {
    ...
    nbitblock = (NBLOCK + BIT2BLK - 1) / BIT2BLK; //除以BIT2BLK，向上取整（其实nbitblock=1
    nextbno = 2 + nbitblock;   //位图从2号数据块开始，位图后面下一个可用的数据块号是nextbno（nextbno=3
    ...
    for(i = 0; i < nbitblock; ++i) {
        memset(disk[2+i].data, 0xff, NBLOCK/8);  //这个函数没找到，难道是fsformat.bat里的？最后一个参数是干嘛的
    }                                            //1K个数据块，只需要128字节的位图，而一个数据块4K字节。。只需一块
    if(NBLOCK != nbitblock * BY2BLK) { //这里是不是错了。。。BIT2BLK
        diff = NBLOCK % BY2BLK / 8;    //这里是不是错了。。。BIT2BLK，但结果竟然奇迹地对了！
        memset(disk[2+(nbitblock-1)].data+diff, 0x00, BY2BLK - diff); //最后一个参数是要填的字节数？没错
    }
