...
unmap_block(u_int blockno)
{
	...
	if(block_is_dirty(blockno)&&!block_is_free(blockno)){
		ide_write(0, blockno * SECT2BLK, (void *)diskaddr(blockno), SECT2BLK);
	}
...
alloc_block_num(void)
{
	...
	for (blockno = 3; blockno < super->s_nblocks; blockno++) {
		if (bitmap[blockno / 32] & (1 << (blockno % 32))) {	//the block is free
			bitmap[blockno / 32] &= ~(1 << (blockno % 32)); //其余位不变，对应位清零
			write_block(blockno / BIT2BLK); // write to disk.  //难道是要写回硬盘上的bitmap苦块？那块号也不对啊
			return blockno;
		}
	}
	// no free blocks.
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	// nblock: how many blocks this dir have.
	nblock = dir->f_size / BY2BLK;

	for (i = 0; i < nblock; i++) {
		// read the i'th block of the dir.
		if ((r = file_get_block(dir, i, &blk)) < 0) {
			return r;
		}

		// find target file by file name in all files on this block.
		f = blk;
		for (j = 0; j < FILE2BLK; j++) {
			if (strcmp((char *)f[j].f_name, name) == 0) {
				*file = &f[j]; // set the result to *file.
				f[j].f_dir = dir; // set f_dir field.
				return 0;
			}
		}
	}
