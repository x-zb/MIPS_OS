...
umain(void)
{
	...
	if (r == 0) {
		...
		// as the parent dups and closes it (there's a close implicit in dup).？？？？什么狗屁注释
		//                                                                是说第二次dup(p[0],10)里close(10)吗？
		// The ref count for p[1] is 1.
		// Thus the ref count for the underlying pipe structure
		// will toggle between 3 and 4.
		//
		// If a clock interrupt catches close between unmapping  是说第二次dup(p[0],10)里close(10)吗？
